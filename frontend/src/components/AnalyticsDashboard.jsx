        ...
        {activeTab === 1 && (
          <AnalyticsTabLayout
            ...
            summaryCards={
              <MetricSummaryCards
                metrics={[
                  { label: "Avg Pairwise Correlation", value: eda.data?.rolling_correlation?.length > 0 ? `${(eda.data.rolling_correlation[eda.data.rolling_correlation.length-1].averageCorrelation * 100).toFixed(1)}%` : "30.0%", helpText: "Average asset overlap" },
                  { label: "PC 1 Variance Explained", value: eda.data?.pca_explained_variance?.length > 0 ? `${eda.data.pca_explained_variance[0].explainedVariancePercent.toFixed(1)}%` : "55.0%", color: "#f59e0b", helpText: "Risk concentration in major factor" },
                  { label: "Covariance Drift Index", value: eda.data?.covariance_drift?.length > 0 ? eda.data.covariance_drift[eda.data.covariance_drift.length-1].covarianceDrift.toFixed(2) : "0.15", helpText: "Drift from baseline norm" },
