...
export function getResponsiveChartHeight(spec, fallback = 350) {
...
export function layoutSankey({ spec, containerWidth, height }) {
  ...
  for (let i = 0; i < nodes.length; i += 1) {
    links.forEach((link) => depths.set(link.target, Math.max(depths.get(link.target) || 0, (depths.get(link.source) || 0) + 1)));
  }
